...
public class Grade_RazredController {
	...
	public ResponseEntity<?> getRazred() {
		return new ResponseEntity<List<Grade_RazredEntity>>((List<Grade_RazredEntity>) razredRepository.findAll(), HttpStatus.OK);
	}
	...
	public ResponseEntity<?> changeRazred(@RequestBody Grade_RazredRegisterDTO changedRazred, @PathVariable Integer id) {
		if (razredRepository.existsById(id)) {
